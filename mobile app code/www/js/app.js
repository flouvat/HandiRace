...
perimetre = 2*Math.PI*rayon*Math.pow(10, -3);
distanceTotale = app.options.distance;
if( (athlete1 = app.options.athlete1nom) == "" ) athlete1 = "athlete 1";
if( (athlete2 = app.options.athlete2nom) == "" ) athlete2 = "athlete 2";
difficulte1 = app.options.athlete1difficulte;
difficulte2 = app.options.athlete2difficulte;
...
app.startGetInfos = function(){
        ...
        if(toggleGetInfos){     // make sure we want to get infos 
            ...
            if(capteur == 1){
                if(distanceParcourue1 < distanceTotale*difficulte1){
                    if ( (perimetre*1000/time*3.6) < 50 ){          // Temporarily hiding a bug - speed can't excess 50km/h
                        distanceParcourue1 += perimetre;
                        vitesse1 = perimetre*1000/time*3.6;
                    ...
                    if(toggleGetChrono2) gagnant = athlete1;     // if player2's chrono is still running, player1 is the winner
                }
                ...
                if(distanceParcourue2 < distanceTotale*difficulte2){
                    if ( (perimetre*1000/time*3.6) < 50 ){          // Temporarily hiding a bug - speed can't excess 50km/h
                        distanceParcourue2 += perimetre;
                        vitesse2 = perimetre*1000/time*3.6;
                    ...
                    if(toggleGetChrono1) gagnant = athlete2;     // if player1's chrono is still running, player2 is the winner
                }
    ...
    intervalID = setInterval(function() {
        var vitesseTemp1, vitesseTemp2;
            if((vitesseTemp1 = compteur1*perimetre/(frequenceUpdateVitesse/1000)*3.6) <= vitesse1) vitesse1 = vitesseTemp1;
            Dom.set('vitesse1', Math.round(vitesse1*100)/100 + " km/h");
            compteur1 = 0;

            if((vitesseTemp2 = compteur2*perimetre/(frequenceUpdateVitesse/1000)*3.6) <= vitesse2) vitesse2 = vitesseTemp2;
            Dom.set('vitesse2', Math.round(vitesse2*100)/100 + " km/h");
            compteur2 = 0;
